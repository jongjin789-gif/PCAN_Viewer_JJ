...
class PCANBasic:
    ...
    def __init__(self):
        ...
        if platform.system() == 'Windows':
            # PyInstaller 단일 실행 파일(--onefile)로 실행 시 임시 폴더에서 로드
            if hasattr(sys, '_MEIPASS'):
                # [TIP] Python 3.8 이상에서 DLL 의존성을 안정적으로 찾도록 경로 명시적 추가
                if sys.version_info >= (3, 8):
                    os.add_dll_directory(sys._MEIPASS)
                dll_path = os.path.join(sys._MEIPASS, "PCANBasic.dll")
                self.__m_dllBasic = windll.LoadLibrary(dll_path)
            else:
                # 일반 Python 환경 로드 시에도 현재 스크립트 경로를 탐색 경로에 추가
                if sys.version_info >= (3, 8):
                    os.add_dll_directory(os.path.dirname(os.path.abspath(__file__)))
                # 일반 Python 환경에서 로드 (시스템 Path 또는 현재 폴더)
                self.__m_dllBasic = windll.LoadLibrary("PCANBasic")
        elif platform.system() == 'Linux':
            # Loads the API on Linux
            if hasattr(sys, '_MEIPASS'):
                dll_path = os.path.join(sys._MEIPASS, "libpcanbasic.so")
                self.__m_dllBasic = cdll.LoadLibrary(dll_path)
            else:
            ...
            #
            if hasattr(sys, '_MEIPASS'):
                dll_path = os.path.join(sys._MEIPASS, "libPCBUSB.dylib")
                self.__m_dllBasic = cdll.LoadLibrary(dll_path)
            else:
